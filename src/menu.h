...
{
protected:
    bool activated;  ///< Menu visibility state
...
public:
    ...
    /// @{
    bool is_activated(void) { return activated; }; ///< Check menu visibility
    void activate(void) { activated = true; }; ///< Show menu
    void deactivate(void) { activated = false; }; ///< Hide menu
    /// @}
...
public:
    ...
    /**
     * @brief Handle cursor movement with viewport adjustment
     * @param dcursor Relative cursor movement (-1/+1 typical)
     */
...
class TutorialMenu : public BasicMenu
{
    ...
    int current_page = 0; ///< Active page index

public:
    /**
     * @brief Construct a new TutorialMenu
     * @param content Paginated text content (vector of pages)
     */
    ...
    /// @name Content Access
    /// @{
    const std::vector<std::string> &get_content() const; ///< Get current page lines
